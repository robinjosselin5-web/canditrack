...
describe('candidateProfileDataController', () => {
  ...
  it('creates and deletes a skill with UUID validation', async () => {
    ...
    await controllers.createCandidateSkillController({ body: { name: 'TypeScript', category: 'PROGRAMMING_LANGUAGES' } } as Request, response)
    expect(serviceMock.createSkillForUser).toHaveBeenCalledWith('user-1', { name: 'TypeScript', category: 'PROGRAMMING_LANGUAGES' })
